...
// FindImageAll 在指定区域内查找匹配的图片模板，支持透明图像处理，返回所有符合条件的坐标。
//
// 参数：
//   - x1, y1: 区域左上角的坐标。
//   - x2, y2: 区域右下角的坐标。当 x2 或 y2 为 0 时，表示使用图像的最大宽度或高度。
...
func FindImageAll(x1, y1, x2, y2 int, template *[]byte, isGray bool, scalingFactor, sim float32, displayId int) []images.Point {
	return nil
}
